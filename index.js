...
function findElementGreaterThan(arr, num) {
  ...
  const BigNum = arr.find((el) => el > num);
  return BigNum;
}
...
function getPositiveArrayValues(arr) {
  ...
  const result = Array.from(iter).filter((el) => el > 0);
  return result;
}
...
function removeAndDouble(arr, startIdx) {
  ...
  // let doubledArray = arr.splice(startIdx);
  // return doubledArray.map((el) => el * 2);

  let doubledArray = arr.slice();
  doubledArray.splice(startIdx);
  return doubledArray.map((el) => el * 2);
}
...
function sumAndPrint(arr) {
  ...
  let sum = arr.reduce((el1, el2) => el1 + el2, 0);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  return f2.filter((el) => el > threshold);
}
...
function wordLengths(arr) {
  ...
  let W = arr.map((word) => word.length);
...
function expandArrayByN(arr, n) {
  ...
  // return arr.flatMap((num) => Number(num).repeat(n));
}
...
function findLongestWord(arr) {
  ...
  return arr.reduce((el1, el2) => (el1 > el2 ? el1 : el2), 0);
}
...
function capitalizeWords(arr) {
  ...
  return arr.map((el) => el.at(0).toUpperCase() + el.slice(1));
}
...
function calculateTotalPrice(arr) {
  ...
  return arr.reduce((el1, el2) => el1 + el2.price, 0);
}
